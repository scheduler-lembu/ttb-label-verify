...
|---|---|---|---|---|---|
| D-1 | **Extraction engine** | Vision-capable LLM (cloud) | One call does OCR + field understanding; best on imperfect images; fits ~5s | Cloud dependency; per-call cost; non-deterministic transcription | Azure OpenAI / Azure Document Intelligence **inside TTB's existing Azure tenant** → avoids the outbound-firewall block |
